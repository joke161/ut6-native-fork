...
function UT.GameUtility:getCurrentDodgeChance()
    if not UT.GameUtility:isInHeist() or not managers.player then
        return 0
    end

    local upgradeValue = (PlayerManager.orig and PlayerManager.orig.upgrade_value) or PlayerManager.upgrade_value
    if not upgradeValue then
        return 0
    end

    local dodge = upgradeValue(managers.player, "player", "passive_dodge_chance", 0)

    local playerUnit = UT.GameUtility:getPlayerUnit()
    if playerUnit and alive(playerUnit) then
        local movement = playerUnit:movement()
        if movement then
            if movement:running() then
                dodge = dodge + upgradeValue(managers.player, "player", "run_dodge_chance", 0)
            elseif movement:crouching() then
                dodge = dodge + upgradeValue(managers.player, "player", "crouch_dodge_chance", 0)
            elseif movement:zipline_unit() then
                dodge = dodge + upgradeValue(managers.player, "player", "on_zipline_dodge_chance", 0)
            elseif managers.player:has_category_upgrade("player", "mrwi_dodge_chance") then
                dodge = dodge + upgradeValue(managers.player, "player", "mrwi_dodge_chance", 0)
            end
        end
    end
